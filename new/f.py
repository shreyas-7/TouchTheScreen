...
def collection_with_normalisation() :
    ...
    arduinoString_x = m.arduinoData.readline()
    print arduinoString_x
    volt_x = np.array([float(x)/m.initial_array_x(x) for x in arduinoString_x.split()])

    arduinoString_y = m.arduinoData.readline()
    print arduinoString_y
    volt_y = np.array([float(y)/m.initial_array_x(y) for y in arduinoString_y.split()])
