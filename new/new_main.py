...
for i in range(20) :
	memory_array_x.append(False)
	memory_array_y.append(False)
...
def process_values(x_array,y_array) :
	...
	for i in range(horizontal_led) :
		if i == 0 :
			if  (abs(x_array[i]-100) > x_threshold and x_array[i] < x_array[i+1]) :
				x_fingers += 1
				x_res.append(i)

		elif i == horizontal_led-1 :
			if (abs(x_array[i]-100) > x_threshold and x_array[i] < x_array[i-1]):
				x_fingers += 1
				x_res.append(i)

		else :
			if (abs(x_array[i]-100) > x_threshold and x_array[i] < x_array[i-1] and x_array[i] < x_array[i+1]):
				x_fingers += 1
				x_res.append(i)
	...
	for i in range(vertical_led) :
		if i == 0 :
			if  (abs(y_array[i]-100) > y_threshold and y_array[i] < y_array[i+1]):
				y_fingers += 1
				y_res.append(i)

		elif i == vertical_led-1 :
			if (abs(y_array[i]-100) > y_threshold and y_array[i] < y_array[i-1]):
				y_fingers += 1
				y_res.append(i)

		else :
			if (abs(y_array[i]-100) > y_threshold and y_array[i] < y_array[i-1] and y_array[i] < y_array[i+1]):
				y_fingers += 1
				y_res.append(i)
	...
	if x_fingers == 0 and y_fingers == 0 :
		...
		return 1 , x_res, y_res
	 	...
	 	return 2, x_res, [y_res[0],y_res[0]]

	elif x_fingers == 1 and y_fingers == 2 :
	 	return 2, [x_res[0],x_res[0]], y_res

	elif x_fingers == 3 and y_fingers == 3 :
...
def collection_without_normalisation() :
	# collection of data from arduino, without normalisation
    arduinoString_x = arduinoData.readline()
    volt_x = [float(x) for x in arduinoString_x.split()]

    arduinoString_y = arduinoData.readline()
    volt_y = [float(y) for y in arduinoString_y.split()]
...
def collection_with_normalisation() :
    ...
    print "collecting " 
    for i in range(32) :
    	volt_x.append(float(arduino_x[i])/initial_array_x[i])
    	if i < 16 :
    		volt_y.append(float(arduino_y[i])/initial_array_y[i])
    print "volt_x = " , volt_x
    print "volt_y = " , volt_y	
	# for i in range(32) :
...
# for i in range(10) :
# 	if mn == 0 :
# 		abcd = collection_without_normalisation()
# 	if len(abcd[0]) == 32 :
# 		mn = 1
# 		initial_array = collection_without_normalisation()
# 		for i in range(32) :
# 			initial_array_x[i] += initial_array[0][i]
# 			if i < 16 :
# 				initial_array_y[i] += initial_array[1][i]
if len(initial_array_x) == 16 :
	arduinoData.readline()
	temp = initial_array_x
	initial_array_x = initial_array_y
	initial_array_y = temp

print "len == ",len(initial_array_x)
...
for z in range(9) :
	...
	for i in range(32) :
		...
		if i < 16 :
			initial_array_y[i] += initial_array[1][i]
print "collected_initial_arrays"

for i in range(32) :
	initial_array_x[i] = float(initial_array_x[i])/1000.0
	if i < 16 :
		initial_array_y[i] = float(initial_array_y[i])/1000.0

print "initial_array_x = ", initial_array_x
print "initial_array_y = ", initial_array_y

def main () :
	# an array of arrays
	memory_array_x = [False for i in range(20)]
	memory_array_y = [False for i in range(20)]
	...
	while True :
		...
		# elif no_of_fingers == 1 or no_of_fingers == 2:
			...
			print "calling one finger function"
			memory_array_x[i] = x_pixels
			memory_array_y[i] = y_pixels
			print x_pixels[0],y_pixels[0]
			# mouse.position(int(x_pixels[0]),int(y_pixels[0]))
			pyautogui.moveTo(x_pixels[0],y_pixels[0])
		...
		# print " memory_array_x = ", memory_array_x
		# print " memory_array_y = ", memory_array_y
		i += 1
